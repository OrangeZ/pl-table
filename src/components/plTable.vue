<template>
  <div class="plTableBox" ref="tableBox">
    <div
      ...
      :style="{ maxHeight: heightSwitch ? maxHeight + 'px' : '' }"
    >
      <el-table
        ref="singleTable"
        style="width: 100%"
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      ...
      if (this.recordTableSelect) {
        ...
        // 数组去重然后再赋值
        this.tableSelectData = JSON.parse(
          JSON.stringify(this.removal(this.tableSelectData))
        );
        // 这就是整个表格当前选中项（包含分页）
        this.$emit(
          "table-select-change",
    ...
    toggleRowSelection(row, isCheck) {
      if (row) {
        this.$refs.singleTable.toggleRowSelection(row, isCheck);
      }
    },
    // 用于多选表格，切换某一行的选中状态
    ...
    getSummaries(param) {
      if (typeof this.summaryMethod === "function" && this.summaryMethod) {
        ...
        columns.forEach((column, index) => {
          ...
          // 需要合并的选项
          const flas = this.totalOption.filter(
    ...
    setHeight() {
      if (this.$refs.tableBox) {
        let myPagination = this.$refs.myPagination
          ? this.$refs.myPagination.offsetHeight
          : 0;
                  ...
                  if (parseInt(item.style.height)) {
                    if (item.children[0]) {
                      ...
                      if (tablebodyH > 0) {
                        ...
                        // 如果出现滚动条(就要把表体高度加上9,   9就是滚动条样式的高度，) 9
                        this.maxHeight =
                          tableBox - myPagination - tablebodyH + 9;
                        // 如果没数据 就加上提示的高度
